...
// node_modules/jotai-optics/dist/focusAtom.js
...
function focusAtom(baseAtom, callback) {
  return memo2(() => {
    const focus = callback(optic2());
    const derivedAtom = atom((get3) => {
      const base = get3(baseAtom);
      return base instanceof Promise ? base.then((v) => getValueUsingOptic(focus, v)) : getValueUsingOptic(focus, base);
    }, (get3, set3, update) => {
      const newValueProducer = isFunction(update) ? modify2(focus)(update) : set2(focus)(update);
      const base = get3(baseAtom);
      return set3(baseAtom, base instanceof Promise ? base.then(newValueProducer) : newValueProducer(base));
    });
    return derivedAtom;
  }, baseAtom, callback);
}
var getValueUsingOptic = (focus, bigValue) => {
  if (focus._tag === "Traversal") {
    const values = collect2(focus)(bigValue);
    return values;
  }
  if (focus._tag === "Prism") {
    const value2 = preview2(focus)(bigValue);
    return value2;
  }
  const value = get2(focus)(bigValue);
  return value;
};
export {
  focusAtom
};
//# sourceMappingURL=jotai-optics.js.map
