...
// node_modules/react-viewport-list/lib/index.js
var require_lib = __commonJS({
  "node_modules/react-viewport-list/lib/index.js"(exports) {
    ...
    var normalizeValue = (min, value, max = Infinity) => Math.max(Math.min(value, max), min);
    var getDiff = (value1, value2, step) => Math.ceil(Math.abs(value1 - value2) / step);
    var useIsomorphicLayoutEffect = IS_SSR ? react_1.useEffect : react_1.useLayoutEffect;
    var generateArray = (from, to, generate) => {
      const array = [];
    ...
    var findElement = ({ fromElement, toElement, fromIndex, asc = true, compare }) => {
      let index = fromIndex;
    ...
    var findNearestScrollableElement = (propName, node) => {
    ...
    var getStyle = (propName, size, marginTop = 0) => ({
      padding: 0,
      margin: 0,
      border: "none",
      visibility: "hidden",
    ...
    var ViewportListInner = ({ items = [], count, children, viewportRef, itemSize = 0, itemMargin = -1, overscan = 1, axis = "y", initialIndex = -1, initialAlignToTop = true, initialOffset = 0, initialDelay = -1, initialPrerender = 0, onViewportIndexesChange, overflowAnchor = "auto", withCache = true, scrollThreshold = 0, renderSpacer = ({ ref: ref2, style }) => (0, jsx_runtime_1.jsx)("div", { ref: ref2, style }), indexesShift = 0, getItemBoundingClientRect = (element) => element.getBoundingClientRect() }, ref) => {
      ...
      return (0, jsx_runtime_1.jsxs)(react_1.Fragment, { children: [renderSpacer({ ref: topSpacerRef, style: topSpacerStyle, type: "top" }), (!!count || !!items.length) && generateArray(startIndex, endIndex + 1, withCount ? children : (index) => children(items[index], index, items)), renderSpacer({ ref: bottomSpacerRef, style: bottomSpacerStyle, type: "bottom" })] });
    };
    exports.ViewportList = (0, react_1.forwardRef)(ViewportListInner);
  }
});
export default require_lib();
//# sourceMappingURL=react-viewport-list.js.map
